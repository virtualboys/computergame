...
{	
 			...
 			if(Input.GetKeyDown(KeyCode.E))
        		{
        			aSource.pitch = Random.Range(0.8f,1f);
 		    		aSource.PlayOneShot(sfxCoin,1f);
 		        	instCoin = Instantiate(coin, itemSpawnLoc.position, Quaternion.identity);
        			instCoin.GetComponent<Rigidbody>().AddForce(transform.forward * 1);
        		}
 	}
    void FixedUpdate() {
		speed = walkSpeed;
        ...
        if (grounded) {
            ...
            if (!Input.GetButton("Jump"))
            {
                ...
                aSource.PlayOneShot(sfxBounce,0.2f);
            }
        }
        else {
    ...
    void OnControllerColliderHit (ControllerColliderHit hit) {
        contactPoint = hit.point;
            if(hit.gameObject.tag == "prop_billybouncer")
        	{
        	...
        	if(hit.gameObject.tag == "zone_coin")
        	{
        		//if(Input.GetKeyDown(KeyCode.E))
        		//{
        			//Instantiate(coin, itemSpawnLoc.localPosition, Quaternion.identity);
        			//coin.GetComponent<Rigidbody>().AddForce(transform.forward * thrust);
        		//}
        	}
    }
